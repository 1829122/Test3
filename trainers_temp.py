...
class Trainer:
    ...
    def get_sample_scores(self, epoch, pred_list):
        ...
        print(f"Epoch : {post_fix["Epoch"]}")
        print(f"Hit@1 : {post_fix["HIT@1"]}")
        print(f"Hit@5 : {post_fix["HIT@5"]}, NDCG@5 : {post_fix["NDCG@5"]}")
        print(f"Hit@10: {post_fix["HIT@10"]}, NDCG@10: {post_fix["NDCG@10"]}")
    ...
    def get_full_sort_score(self, epoch, answers, pred_list, mode):
        ...
        # print(post_fix)
        # for k in post_fix.keys():
        #     print(f"{k}: {post_fix[k]}")
        print(f"Epoch : {post_fix["epoch"]}")
        print(f"Hit@5 : {post_fix["HIT@5"]}, NDCG@5 : {post_fix["NDCG@5"]}")
        print(f"Hit@10: {post_fix["HIT@10"]}, NDCG@10: {post_fix["NDCG@10"]}")
        print(f"Hit@20: {post_fix["HIT@20"]}, NDCG@20: {post_fix["NDCG@20"]}")
...
class CoSeRecTrainer(Trainer):
    ...
    def mutual_kl_divergence(self, log_probs1, log_probs2):
        ...
        # pq = 0
        # pq = (p2 * (torch.log(p2) - torch.log(p1))).sum(dim=-1) # 원래 이게 맞음
        pq = (p2 * (torch.log(p2) - p1)).sum(dim=-1) # 잘못된 식
