...
class SASRecModel(nn.Module):
    def __init__(self, args):
        ...
        self.position_embedding2 = nn.Embedding(args.max_seq_length, args.hidden_size)
        self.item_encoder = Encoder(args)
        self.item_encoder2 = Encoder(args)
        self.LayerNorm = LayerNorm(args.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(args.hidden_dropout_prob)
        self.gate = nn.Linear(args.hidden_size * 2, 2)
        self.args = args
    ...
    def add_position_embedding(self, sequence, mode):
        ...
        if mode == 'n':
            ...
            position_embeddings = self.position_embedding2(position_ids)
    ...
    def forward(self, input_ids):
        ...
        sequence_emb = self.add_position_embedding(input_ids, 'n')

        item_encoded_layers = self.item_encoder(sequence_emb, extended_attention_mask, output_all_encoded_layers=True)

        n_sequence_output = item_encoded_layers[-1]
        
        sequence_emb = self.add_position_embedding(input_ids, 's')

        item_encoded_layers = self.item_encoder2(sequence_emb, extended_attention_mask, output_all_encoded_layers=True)

        s_sequence_output = item_encoded_layers[-1]
        gate_output = torch.cat([n_sequence_output, s_sequence_output], dim = -1)
        gate_score = self.gate(gate_output)
        # 마지막 차원에 대해 softmax -> (B, L, 2)
        alpha = torch.softmax(gate_score, dim=-1)

        # 브로드캐스팅을 통해 최종 가중합 (B, L, H)
        fused_output = alpha[..., 0].unsqueeze(-1) * n_sequence_output + alpha[..., 1].unsqueeze(-1) * s_sequence_output

        
        return n_sequence_output, s_sequence_output, fused_output
